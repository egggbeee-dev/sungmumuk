document.addEventListener('DOMContentLoaded', async function () {
  ...
  async function loadPosts(filter = "") {
      try {
          const response = await fetch(`/free/posts?filter=${filter}`);
  ...
  function renderPosts() {
    ...
    posts.forEach(post => {
        ...
        titleLink.href = `/free_post.html?id=${post.id}`;
  ...
  async function editPost(post) {
      ...
      try {
          const response = await fetch(`/free/posts/${post.id}`, {
              method: 'PUT',
  ...
  async function deletePost(postId) {
      ...
      try {
          const response = await fetch(`/free/posts/${postId}`, {method: 'DELETE'});
          if (response.ok) {
              alert('게시글이 삭제되었습니다.');
              await loadPosts(filterSelect.value);
          } else {
  ...
  async function searchPosts() {
      ...
      try {
        const response = await fetch(`/free/search?query=${encodeURIComponent(query)}`);
