...
router.post('/free_new/posts', ensureAuthenticated, upload.single('image'), async (req, res) => {
  ...
  try {
      const [result] = await pool.query(query, [title, content, category, imageUrl, userId]); // 콜백함수 없게 써야한다고 해서 일단 바꿨습니다
